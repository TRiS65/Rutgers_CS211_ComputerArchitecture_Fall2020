...
void fillGates(int unknown, struct Gate* result, int** test){
...
//reducing gates
int isOROR(struct Gate* front){
    struct Gate* first = front->next;
    struct Gate* second = first->next;
    struct Gate* third = second->next;
    while(second != NULL){
        if(second->name[0] == 'O'){
            if((strcmp(second->IN1,first->OUT) == 0 && strcmp(second->IN2,first->IN1) == 0) || (strcmp(second->IN1,first->OUT) == 0 && strcmp(second->IN2,first->IN2) == 0) || (strcmp(second->IN2,first->OUT) == 0 && strcmp(second->IN1,first->IN1) == 0) || (strcmp(second->IN2,first->OUT) == 0 && strcmp(second->IN1,first->IN2) == 0)){
                // associative
                strcpy(second->IN1,first->IN1);
                strcpy(second->IN2,first->IN2);
                front->next = first->next;
                free(first);
//                printf("returned second %s %s %s\n", second->IN1,second->IN2,second->OUT);
                return 1;
            }else{
//                printf("distributive? %s %s %s, %s %s %s\n",first->IN1,first->IN2,first->OUT,second->IN1,second->IN2,second->OUT);
                //distributive or not
                while(third != NULL){
                    if(third->name[0] == 'A'&& (strcmp(third->IN1,first->OUT) == 0 && strcmp(third->IN2,second->OUT) == 0)){
//                        printf("distributive!\n");
                        // 4 situations -> b c, a out2
                        if(strcmp(second->IN1,first->IN1) == 0){ // a b, a c, out1 out2
                            strcpy(second->name,"AND");
                            strcpy(third->name,"OR");
                            strcpy(second->IN1,first->IN2);
                            strcpy(third->IN1,first->IN1);
                            front->next = first->next;
                            free(first);
//                            printf("returned 1\n");
                            return 1;
                        }else if(strcmp(second->IN2,first->IN1) == 0){ //ab ca
                            strcpy(second->name,"AND");
                            strcpy(third->name,"OR");
                            strcpy(second->IN2,second->IN1);
                            strcpy(second->IN1,first->IN2);
                            strcpy(third->IN1,first->IN1);
                            front->next = first->next;
                            free(first);
//                            printf("returned 2\n");
                            return 1;
                        }else if(strcmp(second->IN1,first->IN2) == 0){ //ba ac
                            strcpy(first->name,"AND");
                            strcpy(third->name,"OR");
                            strcpy(second->IN1,first->IN1);
                            strcpy(third->IN1,first->IN2);
                            front->next = first->next;
                            free(first);
//                            printf("returned 3\n");
                            return 1;
                        }else if(strcmp(second->IN2,first->IN2) == 0){ //ba ca
                            strcpy(second->name,"AND");
                            strcpy(third->name,"OR");
                            strcpy(second->IN2,second->IN1);
                            strcpy(second->IN1,first->IN1);
...
int isANDAND(struct Gate* front){
    struct Gate* first = front->next;
    struct Gate* second = first->next;
    struct Gate* third = second->next;
//    printf("start at %s %s, %s %s; same = %d\n",first->IN1,first->IN2,second->IN1,second->IN2,strcmp(second->IN1,first->IN2));
    while(second != NULL){
//        printf("while second\n");
        if(second->name[0] == 'A'){
            while(third != NULL){
//                printf("while third\n");
                if(third->name[0] == 'O'&& (strcmp(third->IN1,first->OUT) == 0 && strcmp(third->IN2,second->OUT) == 0)){
//                    printf("processing %s %s, %s %s\n",first->IN1,first->IN2,second->IN1,second->IN2);
                    // 4 situations -> b c , a out2
                    if(strcmp(second->IN1,first->IN1) == 0){ // ab ac
//                        printf("first\n");
                        strcpy(second->name,"OR");
                        strcpy(third->name,"AND");
                        strcpy(second->IN1,first->IN2);
                        strcpy(third->IN1,first->IN1);
                        front->next = first->next;
                        free(first);
...
struct Gate* reduceGates(struct Gate* G){
    struct Gate* temp = malloc(sizeof(struct Gate));
    temp->next = G;
    //printf("%s \n",temp->next->name);
    struct Gate* ptr = temp;
    int result;
    while(ptr->next->next != NULL){
        if(ptr->next->name[0] == 'O'){
            result = isOROR(ptr);
            if(result == 1){
                continue;
            }
        }else if(ptr->next->name[0] == 'A'){
            //printf("find AND\n");
            result = isANDAND(ptr);
            //printf("%d %s \n",result,ptr->next->name);
            if(result == 1){
                continue;
            }
        }else if(ptr->next->name[0] == 'M'){
            ptr->next->numIN = pow(2,ptr->next->numIN);
        }
        ptr = ptr->next;
    }
    if(ptr->next->name[0] == 'M'){
        ptr->next->numIN = pow(2,ptr->next->numIN);
    }
    //printf("%s \n",temp->next->name);
    ptr = temp->next;
...
void printCircuit(struct Gate* head){
    if(head == NULL){printf("empty\n");return;}
    struct Gate* ptr = head;
    while(ptr != NULL){
        printf("%s ",ptr->name);
        if(ptr->numIN == 1){
            printf("%s %s",ptr->IN1,ptr->OUT);
        }else if(ptr->numIN == 2){
            printf("%s %s %s",ptr->IN1,ptr->IN2,ptr->OUT);
        }else{
            printf("%d %s",ptr->numIN,ptr->Multi);
        }
        printf("\n");
        ptr = ptr->next;
    }
...
//    int answer = ValidGuess(G,IN,OUT,test,grey,OutputVal,NumOfOutput,NumOfInput);
    ...
    if(answer == 0){
        ...
        fillGates(unknown,G,test);
        G = reduceGates(G);
        FILE* f2 = fopen(argv[1],"r");
        char line[1000];
        for(int i = 0; i < NumOfOutput + 3; i++){
            fscanf(f2,"%[^\n]\n",line);
            printf("%s\n",line);
        }
        printCircuit(G);
